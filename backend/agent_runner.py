...
async def run_pipeline(session_id, form_data, pdf_path):
    ...
    # ── Plan Agent (LangGraph) ────────────────────────────────
    # Pass curriculum records under the 'courses' key; plan_agent also
    # accepts the raw curriculum_agent dict (it normalises internally).
    ...
    markdown = await plan_agent.run(
        ...
        job_data,
        plan_curriculum,
        form_data,       # includes faculty, year, background, preferences, plan_preferences
        plan_emit,
    )
